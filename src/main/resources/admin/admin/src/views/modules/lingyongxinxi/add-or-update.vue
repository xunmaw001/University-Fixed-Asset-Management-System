...
export default {
	...
	created() {
		this.ruleForm.lingqushijian = this.getCurDate()
	},
	methods: {
		 ...
		 if(crossrefid && crossuserid) {
			...
			this.$http({ 
				...
				if (data && data.code === 0) { 
					 ...
					 this.$http({
					   url: `lingyongxinxi/${!this.ruleForm.id ? "save" : "update"}`,
					   method: "post",
					   data: this.ruleForm
					 }).then(({ data }) => {
					   if (data && data.code === 0) {
						 ...
						 this.parent.lingyongxinxiCrossAddOrUpdateFlag = false;
						 this.parent.search();
